...
navigator.serviceWorker
  ...
  .then(registration => {
     ...
     * 一旦监听的安装事件执行成功，则registration.waiting状态不为null， 然后进入 activate 阶段，在此阶段会清除旧的缓存资源，使用新的缓存资源。
     * 如果在sw.js中监听了 activate 事件，则会执行，完成后，将结果返回给registration。此时registration.active不为null，则为激活成功
     * 
     * 
     * registration对象的api
      registration= {
